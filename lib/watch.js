...
module.exports = function() {
	...
	chokidar.watch(pathsToWatch, {
		ignored: /\.tern-port|flycheck_*|node_modules|\.git|jspm_packages/,
		ignoreInitial: true
	})
	.on("all", _.debounce(function(event, filepath) {
		...
		link(moduleName, modulePath).then(function() {
			...
			// to be done
			fs.outputFileSync(getLinkerCache(), JSON.stringify(data));
		});
